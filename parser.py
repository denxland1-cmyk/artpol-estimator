...
Правила:
- area_m2: общая площадь стяжки в квадратных метрах
- thickness_mm: толщина слоя стяжки в миллиметрах (если указан в см — переведи в мм)
...
async def process_measurement(text: str) -> dict:
    ...
    Полный пайплайн:
    1. Парсим текст → JSON
    2. Если за городом + есть координаты → считаем расстояние
    3. Возвращаем готовый результат
    """
    ...
    if parsed.get("thickness_mm") is None:
